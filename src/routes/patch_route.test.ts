...
describe('PATCH /users/:userId/books/:bookId.json', () => {
  ...
  it('returns 400 if any patch module reports failure', async () => {
    mockAddPatchQueriesLatest.mockReturnValue({ patch: "latest_location", success: false, containedOldPatch:false })
    mockAddPatchQueriesHighlights.mockReturnValue({ patch: "highlights", success: false, containedOldPatch:false })
    mockAddPatchQueriesClassrooms.mockReturnValue({ patch: "classrooms", success: false, containedOldPatch:false })
  ...
  it('returns 412 if patch contains old data', async () => {
    ...
    mockAddPatchQueriesHighlights.mockReturnValue({ patch:"highlights", success: true, containedOldPatch: true })
    mockAddPatchQueriesClassrooms.mockReturnValue({ patch:"classrooms", success: true, containedOldPatch: true })
  ...
  it('runs queued queries and returns 200 on success', async () => {
    mockAddPatchQueriesLatest.mockImplementation(({ queriesToRun }) => {
      ...
      return { patch:"latest_location", success: true, containedOldPatch:false }
    })

    mockAddPatchQueriesHighlights.mockReturnValue({ patch:"highlights", success: true, containedOldPatch: false })
    mockAddPatchQueriesClassrooms.mockReturnValue({ patch:"classrooms", success: true, containedOldPatch: false })
