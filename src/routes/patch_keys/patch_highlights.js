...
module.exports = {
  addPreQueries: ({ body, params, preQueries }) => {
    if ((body.highlights || []).length > 0) {
      ...
      const shareCodes = [
        '-',
        ...new Set(
          body.highlights.map(({ share_code }) => share_code).filter(Boolean),
        ),
      ];
      preQueries.queries.push(`
      ...
      `);
      preQueries.vars = [...preQueries.vars, shareCodes];
    } else {
  ...
  addPatchQueries: ({
    ...
    if ((highlights || []).length > 0) {
      ...
      dbHighlights.forEach(function (dbHighlight) {
        currentHighlightsUpdatedAtTimestamp[getHighlightId(dbHighlight)] =
          util.mySQLDatetimeToTimestamp(dbHighlight.updated_at);
        currentHighlightsHasNote[getHighlightId(dbHighlight)] =
          !!dbHighlight.hasnote;
      });
      ...
      for (let highlight of highlights) {
        if (
          !util.paramsOk(
            highlight,
            ['updated_at', 'spineIdRef', 'cfi'],
            ['color', 'note', 'sketch', 'share_code', 'share_quote', '_delete'],
          )
        ) {
          return getErrorObj(
            `invalid parameters (highlight: ${JSON.stringify(highlight)})`,
          );
        }
        ...
        if (
          ...
          (dbHighlightShareCodes || []).some(
            ({ user_id, book_id, spineIdRef, cfi, share_code }) =>
              (userId != user_id ||
                bookId != book_id ||
        ...
        if (
          (currentHighlightsUpdatedAtTimestamp[getHighlightId(highlight)] ||
            0) > highlight.updated_at
        ) {
        ...
        if (highlight._delete) {
          if (currentHighlightsHasNote[getHighlightId(highlight)]) {
            queriesToRun.push({
              query:
                'UPDATE `highlight` SET deleted_at=? WHERE user_id=? AND book_id=? AND spineIdRef=? && cfi=? AND deleted_at=?',
              vars: [
                now,
                userId,
                bookId,
            ...
            queriesToRun.push({
              query:
                'DELETE FROM `highlight` WHERE user_id=? AND book_id=? AND spineIdRef=? AND cfi=? AND deleted_at=? AND updated_at<=?',
              vars: [
                userId,
                bookId,
                highlight.spineIdRef,
          ...
          currentHighlightsUpdatedAtTimestamp[getHighlightId(highlight)] != null
        ) {
          queriesToRun.push({
            query:
              'UPDATE `highlight` SET ? WHERE user_id=? AND book_id=? AND spineIdRef=? AND cfi=? AND deleted_at=?',
            vars: [
              highlight,
              userId,
              bookId,
