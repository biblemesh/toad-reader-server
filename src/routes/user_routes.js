...
module.exports = function (
  ...
  const sendSharePage = async ({
    ...
    const urlWithoutEditing = `${util.getProtocol({ req })}://${req.headers.host}${req.originalUrl
      .replace(/([?&])editing=1&?/, '$1')
      .replace(/iniframe=1&?/, '')
      .replace(/[?&]$/, '')}`;
    ...
    const bookImageUrl = /^epub_content\/covers\//.test(coverHref || ``)
      ? `${frontendBaseUrl}/${coverHref}`
      : `${frontendBaseUrl}/epub_content/covers/book_${book_id}.png`; // this also was the old way of doing things

    let sharePage = fs
      .readFileSync(__dirname + '/../templates/share-page.html', 'utf8')
      .replace(
        /{{page_title}}/g,
        i18n('Quote from {{title}}', { title: title }, { locale }),
      )
      ...
      .replace(
        /{{read_here_url}}/g,
        `${frontendBaseUrl}/#/book/${book_id}/#${spineIdRef ? encodeURIComponent(JSON.stringify({ latestLocation: { spineIdRef, cfi } })) : ``}`,
      )
    ...
    if (note) {
      ...
      sharePage = sharePage.replace(/{{sharer_class}}/g, 'hidden');
    }
  ...
  app.get('/book/:bookId', util.setIdpLang(), (req, res, next) => {
  ...
  app.get(
    ...
    function (req, res) {
      ...
      res.sendFile(
        path.join(process.cwd(), process.env.APP_PATH || '/index.html'),
      );
    },
  );
  ...
  app.get(
    '/users/:userId/books/:bookId.json',
    ensureAuthenticatedAndCheckIDP,
    function (req, res, next) {
      if (parseInt(req.params.userId, 10) !== req.user.id) {
        log(
          [
            'Forbidden: userId in request does not match login',
            req.params.userId,
            req.user.id,
          ],
          3,
      ...
      util
        .hasAccess({ bookId: req.params.bookId, req, log, next })
        .then((accessInfo) => {
          if (!accessInfo) {
          ...
          const hasAccessToEnhancedTools =
            ['ENHANCED', 'INSTRUCTOR'].includes(version) &&
            enhancedToolsExpiresAt > Date.now();
          ...
          const getBookUserData = (classrooms = []) => {
            ...
            const vars = {
              ...
              instructorClassroomUids: [
                '',
                ...classrooms
                  .filter(({ role }) => role === 'INSTRUCTOR')
                  .map(({ uid }) => uid),
              ], // Blank string added to prevent error in case there are none
              defaultClassroomUid,
            };
        ...
        `);
          ...
          `);
            ...
            log([
              'Look up latest location, highlights and classrooms',
              req.params.userId,
              req.params.bookId,
            ]);
            global.connection.query(
              ...
              (err, results) => {
                ...
                if (latestLocations[0]) {
                  ...
                  bookUserData.updated_at = util.mySQLDatetimeToTimestamp(
                    latestLocations[0].updated_at,
                  );
                }
                ...
                if (hasAccessToEnhancedTools || isPublisher) {
                  ...
                  tools = tools.filter((tool) => {
                    ...
                    if (te_uid) {
                      const isUpdateEngagementToolType =
                        ...
                        (['QUESTION'].includes(toolType) && !isDiscussion);
                      ...
                      if (isUpdateEngagementToolType) {
                        if (toolsByUid[uid].engagement) {
                          ...
                          log(
                            [
                              'Unexpected duplicate entires for update engagement tool type',
                              toolType,
                              uid,
                              req.params.userId,
                            ],
                      ...
                      if (tea_question_index != null) {
                        ...
                        toolEngagement.answers[parseInt(tea_question_index)] =
                          parseInt(tea_choice_index);
                      }
                    }
                  ...
                  if (scheduleDates) {
                    util
                      .compileScheduleDateItemsTogether({ scheduleDates })
                      .forEach((scheduleDate) => {
                        util.convertMySQLDatetimesToTimestamps(scheduleDate);
                        classroomsByUid[
                          scheduleDate.classroom_uid
                  ...
                  util.convertJsonColsFromStrings({
                    tableName: 'highlight',
                    rows: instructorHighlights,
                  });
                  instructorHighlights.forEach((highlight) => {
                    classroomsByUid[
                      highlight.classroom_uid
                    ].instructorHighlights.push(highlight);
          ...
          `,
              ...
              (err, classrooms) => {
                ...
                if (
                  !classrooms.some(({ uid }) => uid === defaultClassroomUid)
                ) {
  ...
  app.get(
    ...
    async (req, res, next) => {
      ...
      if (!accessInfo) {
        log(
          [
            'Forbidden: user does not have access to this book and so a cookie was not created',
          ],
          3,
        );
        res.status(403).send({ error: 'Forbidden' });
  ...
  app.get(
    ...
    async (req, res, next) => {
      if (process.env.IS_DEV) {
        res
          .status(404)
          .send({ error: '/classroom_query_string does not work on dev' });
        return;
      }
      ...
      if (!hasAccess) {
        log(
          [
            'Forbidden: user does not have access to this classroom and so a cookie was not created',
          ],
          3,
        );
        res.status(403).send({ error: 'Forbidden' });
  ...
  const getDeletionConfirmationCodeAndIdp = async (req, res, next) => {
    ...
    if (
      !req.user.isAdmin &&
      (req.body.expires_at < now || req.body.expires_at > now + 1000 * 60 * 60)
    ) {
      log(
        [
          'Invalid expires_at - must be ms timestamp within the next hour',
          req.body,
        ],
        3,
      );
    ...
    return {
      ...
      code: crypto
        .createHash('sha256')
        .update(`${req.user.id} ${req.body.expires_at} ${internalJWT}`)
        .digest('hex')
        .slice(0, 6),
    };
  };
  ...
  app.post(
    '/request-deletion-confirmation-code',
    ensureAuthenticatedAndCheckIDP,
    async (req, res, next) => {
      ...
      const { idp, code } = await getDeletionConfirmationCodeAndIdp(
        req,
        res,
        next,
      );
  ...
  app.post(
    ...
    async (req, res, next) => {
      if (
        !(
          req.user.isAdmin &&
          util.paramsOk(req.body, ['userIdToDelete'], ['AMPLITUDE_API_KEY'])
        ) &&
      ...
      log(
        [
          `User deletion request - attempting...`,
          userToDelete.id,
          `Executed by: ${req.user.id}`,
        ],
        1,
      );
      ...
      try {
        if (!idp.actionEndpoint)
          throw new Error(`idp.actionEndpoint not setup`);
        ...
        if (response.status === 400) {
          ...
          throw new Error(
            `Invalid 400 response from actionEndpoint (permanently-delete-user)`,
          );
        }

        if (response.status !== 200) {
          throw new Error(
            `Invalid response from actionEndpoint (permanently-delete-user)`,
          );
        }
        ...
        if (success !== true) {
          throw new Error(
            `Missing { success: true } in response from actionEndpoint (permanently-delete-user)`,
          );
        }

        log(
          [
            'Success from actionEndpoint (permanently-delete-user)',
            userToDelete,
          ],
          1,
        );
        ...
        log(
          [
            'POST to actionEndpoint (permanently-delete-user) failed; will send email to IDP instead',
            err,
            userToDelete,
          ],
          3,
      ...
      if (req.body.AMPLITUDE_API_KEY) {
        try {
          ...
          log(
            [
              `POST to amplitude to delete user data (userId: ${userToDelete.id}, idpId: ${userToDelete.idp_id}) failed`,
            ],
            3,
          );
        }
      ...
      log(
        [
          `User deletion request - successful`,
          userToDelete.id,
          `Executed by: ${req.user.id}`,
        ],
        1,
