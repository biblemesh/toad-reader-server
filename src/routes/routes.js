...
module.exports = function (
  ...
  var getAssetFromS3 = function (
    ...
    var urlWithoutQuery = req.originalUrl
      .replace(/(\?.*)?$/, '')
      .replace(/^\/book/, '')
      .replace(/%20/g, ' ');
    if (!tryWithoutDecode) {
  ...
  app.get('/favicon.ico', function (req, res, next) {
    ...
    var getFallbackIco = function () {
      ...
      var staticFile = path.join(
        process.cwd(),
        (process.env.IS_DEV ? '/src' : '') + '/images/favicon.ico',
      );
  ...
  app.get('/epub_content/**', (req, res, next) => {
    if (!process.env.USE_DEVELOPMENT_S3) return next();
  ...
  app.get('*', ensureAuthenticated, async (req, res, next) => {
    const urlWithoutQuery = req.originalUrl
      .replace(/(\?.*)?$/, '')
      .replace(/^\/book/, '')
      .replace(/%20/g, ' ');
    const urlPieces = urlWithoutQuery.split('/');
    const bookId = parseInt(
      (urlPieces[2] || '0').replace(/^book_([0-9]+).*$/, '$1'),
    );
    ...
    if (urlPieces[1] == 'epub_content') {
      ...
      if (!accessInfo) {
        ...
        urlPieces.length === 4 &&
        urlPieces[3] === 'book.epub' &&
        req.book
      ) {
        const currentTimestamp = Date.now();
        const currentMySQLDatetime =
          util.timestampToMySQLDatetime(currentTimestamp);
        ...
        global.connection.query(queries.join(';'), vars, (err) => {
          if (err) return next(err);
          getAssetFromS3(req, res, next);
        });
      } else {
