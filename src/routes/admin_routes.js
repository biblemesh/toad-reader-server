...
module.exports = function (app, s3, ensureAuthenticatedAndCheckIDP, log) {
    ...
    form.on('file', async (name, file) => {
      ...
      try {
        ...
        if(
          ...
          && (req.tenantAuthInfo || {}).domain !== util.getIDPDomain(req.hostname || req.headers.host)
        ) {
  ...
  app.all('/subscriptions', async (req, res, next) => {
    ...
    const subscriptions = await util.runQuery({
      ...
      `,
      vars: {
        domain: util.getIDPDomain(req.hostname || req.headers.host),  // they may not be logged in, and so we find this by domain and not idpId
      },
      next,
    })
