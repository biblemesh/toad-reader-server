...
const getImageFileType = (fileNameOrPath) => {
  ...
  const fileType = /jpeg/.test(mimeType)
    ? `jpeg`
    : /webp/.test(mimeType)
      ? `webp`
      : `png`;
...
const getAdjustedImageBody = async (path, fileType, width, height) => {
  ...
  const result = await img.getBufferAsync(
    fileType === `jpeg` ? Jimp.MIME_JPEG : Jimp.MIME_PNG,
  );
  return result;
};
...
const processUploadedFile = async (
  ...
  try {
    ...
    const { title, author, isbn, coverHref, spines, success } = await parseEpub(
      { baseUri: toUploadDir, log },
    );
...
module.exports = function (app, ensureAuthenticatedAndCheckIDP) {
    ...
    async (req, res, next) => {
      ...
      form.on('file', async (name, file) => {
        ...
        return processUploadedFile(name, file, {
          ...
          res_send: res.send.bind(res),
          res_status: res.status.bind(res),
          res_set: res.set.bind(res),
          res_write: res.write.bind(res),
          res_end: res.end.bind(res),
          epubFilePaths,
          next,
          tmpDir,
        });
  ...
  app.post('/audiobook', async (req, res, next) => {
    ...
    if (
      ...
      !util.paramsOk(
        req.body.book,
        ['audiobookInfo'],
        ['id', 'title', 'author', 'isbn', 'coverHref'],
      ) ||
    ...
    if (!req.body.book.id) {
      // extra stuff for initial insert
  ...
  app.post(
    ...
    async (req, res, next) => {
      ...
      if (
        bookId !== `new` &&
        (bookIdpRows.length !== 1 || bookRows.length !== 1)
      ) {
      ...
      fs.mkdir(tmpDir, (err) => {
        ...
        form.on('file', async (name, file) => {
          const isCoverImage = /^image\//.test(
            mime.getType(file.originalFilename),
          );

          if (processedOneFile) return;
          processedOneFile = true;

          const extension = (file.originalFilename.match(/\.[^.]+$/gi) ||
            [])[0];
          ...
          if (isCoverImage) {
            ...
            body = await getAdjustedImageBody(
              file.path,
              fileType,
              ...(isAudiobook ? [800, 800] : [480]),
            );
          } else {
            if (
              Math.ceil(file.size / 1024 / 1024) > MAX_AUDIOBOOK_FILE_MEBIBYTE
            ) {
              res.status(400).send({
                errorType: 'file_too_large',
            ...
            const metadata = await mm.parseStream(
              ...
              { mimeType: mime.getType(key), size: body.byteCount },
              { duration: true },
            );
  ...
  app.post(
    ...
    async (req, res, next) => {
      ...
      if (
        !util.paramsOk(req.body, ['subscriptions']) ||
        req.body.subscriptions.some(
          (subscription) => !util.paramsOk(subscription, ['id', 'version']),
        )
      ) {
        log(['Invalid parameter(s)', req.body], 3);
        res.status(400).send();
      ...
      const [currentSubscriptionBookRows, bookOnIdp, subscriptionOnIdp] =
        await util.runQuery({
          query: `
      ...
      currentSubscriptionBookRows.forEach(({ subscription_id }) => {
        if (!subscriptions.some(({ id }) => id === subscription_id)) {
          queries.push(
            `DELETE FROM \`subscription-book\` WHERE subscription_id=? AND book_id=?`,
          );
          ...
          !currentSubscriptionBookRows.some(({ subscription_id, version }) => {
            if (subscription_id === subscription.id) {
              if (subscription.version !== version) {
                queries.push(
                  `UPDATE \`subscription-book\` SET version=? WHERE subscription_id=? AND book_id=?`,
                );
                vars.push(version);
                vars.push(subscription_id);
                vars.push(req.params.bookId);
  ...
  app.post(
    ...
    async (req, res, next) => {
      ...
      req.body.metadataValues.forEach((metadataValue) => {
        if (
          ['title', 'author', 'isbn', 'coverHref'].includes(
            metadataValue.metadata_key_id,
          )
        ) {
          vars.bookUpdate[metadataValue.metadata_key_id] =
            `${metadataValue.value || ``}`;
        } else {
    ...
    async (req, res, next) => {
      ...
      for (let idpIndex = 0; idpIndex < idpRows.length; idpIndex++) {
        ...
        for (let i = 0; i < (req.query.numMonths || 6); i++) {
          ...
          const heading = date.toLocaleString('default', {
            month: 'long',
            year: 'numeric',
          });

          if (new Date(fromDate) < new Date(idpRows[idpIndex].created_at))
            break;
          ...
          const useEnhancedReader =
            idpRows[idpIndex].use_enhanced_reader_at &&
            new Date(idpRows[idpIndex].use_enhanced_reader_at) <
              new Date(toDate);
          const useAudiobooks =
            idpRows[idpIndex].use_audiobooks_at &&
            new Date(idpRows[idpIndex].use_audiobooks_at) < new Date(toDate);
          const useEnhancedReaderOrAudiobook = !!(
            useEnhancedReader || useAudiobooks
          );
          const numActiveUsers = parseInt(
            activeUsersRows[0].numActiveUsers,
            10,
          );
          let totalCostInCents;

          if (
            idpRows[idpIndex].specialPricing === 'OLD' &&
            !useEnhancedReaderOrAudiobook
          ) {
            if (i === 0) {
            ...
            if (i === 0) {
              const costChartRows = [
                ...
                },
                {
                  ...
                  'Interactive eReader / Audiobooks':
            ...
            if (
              numActiveUsers <= 400 &&
              idpRows[idpIndex].specialPricing === 'NON-PROFIT'
            ) {
              totalCostInCents =
                (useEnhancedReaderOrAudiobook ? 280 : 100) * 100;
            } else if (numActiveUsers <= 1000) {
              totalCostInCents =
                (useEnhancedReaderOrAudiobook ? 700 : 250) * 100;
            } else if (numActiveUsers <= 2000) {
              totalCostInCents =
                (useEnhancedReaderOrAudiobook ? 1200 : 400) * 100;
            } else if (numActiveUsers <= 5000) {
              totalCostInCents =
                (useEnhancedReaderOrAudiobook ? 2000 : 750) * 100;
            } else if (numActiveUsers <= 10000) {
              totalCostInCents =
                (useEnhancedReaderOrAudiobook ? 3000 : 1000) * 100;
            } else {
              totalCostInCents = useEnhancedReaderOrAudiobook
                ? 30 * numActiveUsers
                : 10 * numActiveUsers;
            }
          }
          ...
          reportInfo[idpIndex].data.push({
            ...
            rows: [
              {
                'eReader version': useEnhancedReaderOrAudiobook
                  ? `Interactive eReader / Audiobooks`
                  : `Standard eReader`,
  ...
  app.post(
    ...
    async (req, res, next) => {
      ...
      `,
        ...
        ...toolRowsToCopy.map(
          (tool, idx) => `INSERT INTO tool SET :tool_insert_${idx}`,
        ),
      ...
      log(
        `Copy tools (from classroom ${sourceClassroomUid} to ${destinationClassroomId}) was successful` +
          (req.body.clearExistingToolsFirst
            ? ` (after existing ${destinationClassroomId} tools were deleted)`
            : ``),
      );
  ...
  app.get(
    ...
    async (req, res, next) => {
      ...
      if (user) {
        const [subscriptionInstances, books, interactiveActivity] =
          await util.runQuery({
            queries: [
              `
