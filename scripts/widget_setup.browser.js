    ...
    setup: function () {
      ...
      for (var i = 0; i < elsLen; i++) {
        ...
        for (var p in queryParamObj) {
          queryParamArray.push(
            encodeURIComponent(p) + '=' + encodeURIComponent(queryParamObj[p]),
          );
        }
        ...
        var iframeEl = newEl('iframe', {
          ...
          src:
            el.href +
            '&widget=1&parent_domain=' +
            encodeURIComponent(location.host) +
            '&' +
            queryParamArray.join('&'),
          style:
            '' +
            'opacity: 0;' +
            'max-height: ' +
            queryParamObj.maxheight +
            ';' +
            'height: ' +
            (queryParamObj.maxheight.match(/px$/)
              ? Math.min(queryParamObj.maxheight, 200)
              : 200) +
            'px;',
        });
        ...
        spinnerEl.innerHTML =
          '<div class="erasereader-bounce1"></div><div class="erasereader-bounce2"></div><div class="erasereader-bounce3"></div>';

        var divEl = newEl('div', {
          className:
            'erasereader-widget-div' +
            (queryParamObj.theme == 'night-theme'
              ? ' erasereader-widget-div-dark'
              : ''),
          style: '' + 'width: ' + queryParamObj.width + ' !important;',
        });
            ...
            switch (data.action) {
              ...
              case 'setReference':
                ...
                var refElA = newEl('a', {
                  ...
                  target: '_blank',
                  href: iframeEl.src
                    .replace(/&widget=1.*$/, '&flash=1')
                    .replace(/^https?:\/\/[^/]+/, event.origin),
                });
                ...
                iframeEl.parentNode
                  .querySelectorAll('.erasereader-widget-reference')
